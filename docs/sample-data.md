...
Real TikTok posts are loaded into the `posts` table by [scripts/ingest-tiktok.ts](../scripts/ingest-tiktok.ts). The discover view reads from this table on mount; search results still come from [src/lib/mock-data.ts](../src/lib/mock-data.ts).

Engagement metrics (likes, comments, shares, views) are real — pulled live from TikTok via `yt-dlp`. Editorial fields (`cluster_id`, `trend_score`, `why_trending`) are randomized per post, deterministic from post id.
...
The `posts` table is created by [supabase/migrations/20260430_add_posts_table.sql](../supabase/migrations/20260430_add_posts_table.sql). Push it with:
...
1. `yt-dlp --dump-json --skip-download` extracts caption, uploader, thumbnail URL, like/comment/share/view counts, upload timestamp.
2. Hashtags are parsed from the caption.
3. A cluster is picked deterministically from [src/lib/clusters.ts](../src/lib/clusters.ts) (six clusters: Sports, Beauty, Gaming/VFX, Food, Comedy/POV, Pop Culture).
4. `trend_score` (65–95) and a `why_trending` JSON blob (summary + signals + related hashtags) are generated from the post id hash.
5. Lone UTF-16 surrogates (broken emoji from TikTok captions) are stripped before the upsert — Postgres `jsonb` rejects them otherwise.
...
- **Freshness.** Re-run the script periodically. There's no cron yet.
